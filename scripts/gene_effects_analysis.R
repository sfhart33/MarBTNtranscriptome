...
# Count overlp among samples
    ...
    all_sample_fusions <- fusions_file %>%
        ...
        filter_all(all_vars(. > 0)) %>%
        nrow() # 15 in all samples - 16 before PEI

    all_healthy_fusions <- fusions_file %>%
        select(all_of(heme_clams)) %>%
        filter_all(all_vars(. > 0)) %>%
        nrow() # 16 in all healthies - 17 before PEI
...
# Correct for fusions in all samples
    ...
    fusion_counts_mean_adj$shared <- c(0, cancer_shared - all_sample_fusions, cancer_shared - all_sample_fusions)
    fusion_counts_mean_adj$usa <- c(0, 0, usabtn_shared - all_sample_fusions)

# t-test
    t.test(as.numeric(filter(fusion_counts, type=="healthy")[,"count"]),as.numeric(filter(fusion_counts, type=="BTN")[,"count"])) # p-value = 0.0001003
...
# plot
  pdf(paste0(output_folder,"/plots/fusion_plot.pdf"))
        ggplot(fusion_counts_mean_adj, aes(x=group,y=mean)) +
            geom_bar(position="dodge", stat="identity", fill = "#969696")+
            geom_bar(aes(x=group,y=usa),position="dodge", stat="identity", fill = "#515151")+
            geom_bar(aes(x=group,y=shared),position="dodge", stat="identity", fill = "black")+
            geom_errorbar(aes(ymin=(mean-sd), ymax=(mean+sd)), width=.2,position=position_dodge(.9))+
            geom_point(data=fusion_counts_adj,
                        aes(x=group,y=as.numeric(adj.count)),
...
pdf(paste0(output_folder,"/plots/cn_exp_plot.pdf"))
    ggplot(cn_exp, aes(cn, -log2FoldChange, fill = sublin)) +
...
# Load steamer sites from genome paper data, join with expression data
    steamer_ins <- paste0(home_dir,"/inputs/steamer_genes.tsv") %>%
        ...
        left_join(
            dplyr::select(
                rownames_to_column(
                    as.data.frame(heme_uBTN), var = "gene"
                ), gene, u_lfc = log2FoldChange, u_padj = padj)
            ) %>%         
        left_join(
            dplyr::select(
                rownames_to_column(
                    as.data.frame(heme_pBTN), var = "gene"
                ), gene, p_lfc = log2FoldChange, p_padj = padj)
            ) %>%
        # mutate(
...
# Count up/down regulated genes
    steamer_upstream_USA <- steamer_ins %>%
        ...
        summarise(
            ...
            # mutate(
            #     U_up_frac = U_up/(U_up+U_dn),
            #     P_up_frac = P_up/(P_up+P_dn)
            # )
